...
Проверим, что данные в бакете object storage доступны, удостоверимся в публичном листинге айтемов бакета с помощью curl:
...
Успешно удалось обнаружить файл `yaqry_dataset.pqt` в указанном s3-бакете
...
Проверим через Yandex Query Language, что удается получить данные из датасета с помощью привязки:
...
Суммарный объем исходящего трафика можно найти прибегнув к фильтрам на основе regex для src_ip и dst_ip. src должен входить во внутреннюю подсеть, dst не должен входить
...
Суммарный объем исходящего трафика можно найти прибегнув к фильтрам на основе regex для src_ip и dst_ip. src должен не входить во внутреннюю подсеть, а dst должен входить
...
В данной работе мы научились анализу данных используя скриптовый язык Yandex Query Language на платформе Yandex Cloud